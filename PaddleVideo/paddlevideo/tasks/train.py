...
def train_model(cfg,
    ...
    writer = LogWriter(osp.join(output_dir, "logs/"+datetime.datetime.now().strftime('%Y-%m-%d %H-%M-%S')))
    best = 0.
    for epoch in range(0, cfg.epochs):
        ...
        for i, data in enumerate(train_loader):
            ...
            if i % cfg.get("log_interval", 10) == 0:
                ...
                writer.add_scalar('train/loss', record_list['loss'].avg, i+epoch*len(train_loader))
                writer.add_scalar('train/lr', record_list['lr'].val, i+epoch*len(train_loader))
        ...
        def evaluate(best):
            ...
            return best, best_flag, record_list["loss"].avg
        ...
        if validate and (epoch % cfg.get("val_interval", 1) == 0
            ...
            with paddle.no_grad():
                best, save_best_flag, val_loss = evaluate(best)
