...
class BaseDataset(Dataset, ABC):
    ...
    def prepare_predict(self, idx):
        """PREDICT: Prepare the data for PREDICT given the index."""
        #Note: For now, paddle.io.DataLoader cannot support dict type retval, so convert to list here
        results = copy.deepcopy(self.info[idx])
        results = self.pipeline(results)
        #unsqueeze label to list
    ...
    def __getitem__(self, idx):
        ...
        if self.test_mode=="predict":
            return self.prepare_predict(idx)
        elif  isinstance(self.test_mode, bool) and self.test_mode:
            return self.prepare_test(idx)
        elif isinstance(self.test_mode, bool) and not self.test_mode:
            return self.prepare_train(idx)
        else:
            raise ValueError("test_mode is True , False or 'predict'.")
