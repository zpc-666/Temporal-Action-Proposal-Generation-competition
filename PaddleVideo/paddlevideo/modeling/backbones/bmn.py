...
class SELayer(paddle.nn.Layer):
    def __init__(self, num_channels, reduction=16):
        ...
        self.se = paddle.nn.Sequential(
            ...
            paddle.nn.Conv1D(num_channels, mid_c, kernel_size=1),
            paddle.nn.ReLU(),
            paddle.nn.Conv1D(mid_c, num_channels, kernel_size=1),
            paddle.nn.Sigmoid()
        )
    def forward(self, x):
        return x*self.se(x)
...
class BMN(paddle.nn.Layer):
    ...
    def __init__(
        ...
        # Base Module
        self.b_rd = paddle.nn.Sequential(
            paddle.nn.Conv1D(
        ...
        self.b_conv1 = paddle.nn.Conv1D(
            ...
            stride=2,
            groups=4,
            weight_attr=init_params('Base_1_w', self.hidden_dim_2d, 3),
            bias_attr=init_params('Base_1_b', self.hidden_dim_2d, 3))
        ...
        self.b_conv3_act = paddle.nn.ReLU()

        self.b_convt4 = paddle.nn.Conv1DTranspose(
            in_channels=self.hidden_dim_1d*2,
            out_channels=self.hidden_dim_1d,
            kernel_size=3,
            stride=2,
            padding=(1, 0),
            groups=4,
            weight_attr=init_params('Base_4_w', self.hidden_dim_1d*2, 3),
            bias_attr=init_params('Base_4_b', self.hidden_dim_1d*2, 3))
        self.b_convt4_act = paddle.nn.ReLU()

        self.b_convt5 = paddle.nn.Conv1DTranspose(
            in_channels=self.hidden_dim_1d*2,
            out_channels=self.hidden_dim_2d,
            kernel_size=3,
            stride=2,
            padding=(1, 0),
            groups=4,
            weight_attr=init_params('Base_5_w', self.hidden_dim_1d*2, 3),
            bias_attr=init_params('Base_5_b', self.hidden_dim_1d*2, 3))
        self.b_convt5_act = paddle.nn.ReLU()

        #self.b_bn = paddle.nn.BatchNorm1D(self.hidden_dim_2d)
        #self.se_b1 = SELayer(self.hidden_dim_1d*2, 32)
        #self.se_b2 = SELayer(self.hidden_dim_2d, 16)
        self.pos_embed = self.create_parameter(shape=(1, self.hidden_dim_2d, tscale), default_initializer=paddle.nn.initializer.Constant(value=0.0))
        ...
        self.ts_conv1 = paddle.nn.Conv1D(
            ...
            padding=1,
            groups=4,
            weight_attr=init_params('TEM_s1_w', self.hidden_dim_2d, 3),
            bias_attr=init_params('TEM_s1_b', self.hidden_dim_2d, 3))
        ...
        self.te_conv1 = paddle.nn.Conv1D(
            ...
            padding=1,
            groups=4,
            weight_attr=init_params('TEM_e1_w', self.hidden_dim_2d, 3),
            bias_attr=init_params('TEM_e1_b', self.hidden_dim_2d, 3))
        ...
        self.p_conv1 = paddle.nn.Conv1D(
            ...
            padding=1,
            groups=1,
            weight_attr=init_params('PEM_1d_w', self.hidden_dim_2d, 3),
            bias_attr=init_params('PEM_1d_b', self.hidden_dim_2d, 3))
    ...
    def forward(self, x):
        ...
        x_b1 = self.b_conv1_act(self.b_conv1(x))
        x = shuffle_layer(x_b1)

        x_b2 = self.b_conv2_act(self.b_conv2(x))
        x = shuffle_layer(x_b2)

        x_b3 = self.b_conv3_act(self.b_conv3(x))
        x = paddle.concat([x_b2, x_b3], axis=1)
        x = shuffle_layer(x)
        #x = self.se_b1(x)

        x_b4 = self.b_convt4_act(self.b_convt4(x))
        x = paddle.concat([x_b1, x_b4], axis=1)
        x = shuffle_layer(x)

        x_b5 = self.b_convt5_act(self.b_convt5(x))
        #x = shuffle_layer(x_b5)
        #x = self.se_b2(x)

        x = shuffle_layer(identity+x_b5)
        #x = x+self.pos_embed
